...
namespace EY.SSA.CommonBusinessLogic.Actors
{
    public class ClientSupervisor : ReceiveActor, IWithUnboundedStash
    {
        ...
        private IActorRef _ClientListActor;
        ...
        private void Initializing()
        {
            ...
            string _ClientListPersistenceId = "ClientList";
            _ClientListActor = Context.ActorOf(Props.Create<ClientListActor>(), _ClientListPersistenceId);
            ...
            Receive<string>(s => {
                if (s == "Send Message")
                    _ClientListActor.Tell(new ClientIdGetListRequest(Self,null));
                _logger.Debug($"Sending Message to:{_ClientListActor.Path.Name} Message:ClientIdGetListRequest");
            ...
            Context.System.Scheduler.ScheduleTellRepeatedly(1000,1000, Self, "Send Message", Self);
